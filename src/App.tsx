...
function App() {
  ...
  const animate = profile.tier !== 'lite' && !profile.saveData && !prefersReducedMotion;
  ...
  return (
    <main>
      ...
      <header className="topbar">
        ...
        <span className="runtime-pill" title="Adaptive experience tier">{online ? '●' : '○'} {profile.tier}</span>
      </header>
        ...
        <div className="chooser-grid">
          <div className="choice-panel">
            ...
            <div className="choice-buttons">
              {moods.map((item) => <button key={item.id} className={mood === item.id ? 'selected' : ''} onClick={() => setMood(item.id)}><span>{item.icon}</span>{item.label}</button>)}
            </div>
          </div>
          <div className="choice-panel">
            ...
            <div className="choice-buttons">
              {energies.map((item) => <button key={item.id} className={energy === item.id ? 'selected' : ''} onClick={() => setEnergy(item.id)}><span>{item.icon}</span>{item.label}</button>)}
            </div>
          </div>
        </div>

        <AnimatePresence mode="wait">
          <motion.article key={`${mood}-${energy}`} className="pairing-result" initial={animate ? { opacity: 0, y: 14 } : false} animate={{ opacity: 1, y: 0 }} exit={animate ? { opacity: 0, y: -8 } : undefined} transition={{ duration: .28 }}>
      ...
      <section className="menu-section" id="menu">
        ...
        <div className="menu-grid">
          {humanMenu.map((item) => <article className="menu-card" key={item.name}><span className="menu-number">{item.accent}</span><div className="dish-icon">🫕</div><h3>{item.name}</h3><p>{item.note}</p><small>{item.ingredients}</small></article>)}
        </div>
        ...
        <div className="menu-grid dog-grid">
          {dogMenu.map((item) => <article className="menu-card" key={item.name}><span className="menu-number">{item.accent}</span><div className="dish-icon">🐾</div><h3>{item.name}</h3><p>{item.note}</p><small>{item.ingredients}</small></article>)}
        </div>
      </section>
      ...
      <section className="promise" id="promise">
        <div className="promise-badge">🐾</div>
